...
# Руководство
### Пример выполнения под unix:
1. Вызов `make -f Makefile_unix` (сборка testNokia.exe).
2. Вызов `make -f Makefile_unix test` (выполнение тестов).
### Пример выполнения под windows:
1. Вызов `make -f Makefile_windows` (сборка testNokia.exe).
2. Вызов `make -f Makefile_windows test` (выполнение тестов).